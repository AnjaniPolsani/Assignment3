...
import ImagesDisplayList from './components/ImagesDisplayList'
...
class App extends Component {
  ...
  onClickImage = id => {
    const {imgDisplay, isTimerRunning} = this.state
    const filteredImage = imagesList.filter(each => each.id === id)
    if (filteredImage[0].imageUrl === imgDisplay) {
      const newImg =
  ...
  render() {
    const {activeTabId, isTimerRunning, score, time, imgDisplay} = this.state

    const filteredProjects = this.getFilteredProjects()
    return (
      <div >
      <ul>
        <MatchGame isTimerRunning={isTimerRunning} score={score} time={time} />
      </ul>
        <div>
          {isTimerRunning ? (
            <div>
              <div >
                <img  src={imgDisplay} alt="match" />
              </div>
